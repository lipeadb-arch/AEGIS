...
**Pacote de IA FUNDAMENTADA (Gemini Free demonstrativo) — EM ANDAMENTO — branch `feat/gemini-ai-grounded` sobre a baseline `fc2827a` (NÃO mergeado, NÃO deployado; sem chave e sem tenant real).** Consolida a IA do AEGIS num provedor ÚNICO com configuração genérica `Ai__*` (Anthropic e a seção `AegisAi` FORA da DI ativa; `AegisAssessmentService` PROVIDER-NEUTRAL implementa `IAiAssessmentService` sobre o `ILLMClient`, com o `GeminiLlmClient` isolado na Infrastructure — nenhum tipo Gemini em controllers/workers/domínio/frontend). Gemini Free DEMONSTRATIVO com modelo estável pinado **`gemini-3.5-flash`** (GA + Free Tier; não alias `-latest`). **Score/pontos/ledger DETERMINÍSTICOS preservados** — a IA só interpreta/redige. **Fronteira de dados do Free Tier por CONFIGURAÇÃO, SEM migration e SEM tocar a entidade `Tenant`** (`Ai__Mode=GeminiFreeDemo` + `Ai__FreeTier__AllowedTenantSlugs__0` = slug do sandbox sintético): tenant fora da allowlist opera no motor determinístico/stub e NUNCA chama a IA externa. Gate + roteadores tenant-scoped protegem TODOS os consumidores (worker documental, Auditor, entrevistas, assessments, advisories, telemetria, Knight); 429/timeout/5xx → fallback seguro. **Parecer documental** exibido no Document Hub (`DocumentMappingDto.EvidenceQuote` + `analysisSummary`, "Analisado com/sem evidência", citação literal + controle + confiança + justificativa; documento sem prova literal → NÃO altera a postura; polling mantido). **Auditor FUNDAMENTADO** num contexto tenant-scoped somente leitura (score/cobertura, lacunas, evidência curta, conectores, recomendações) — cita a origem, separa fato/inferência/recomendação e admite "não há dados suficientes"; nunca envia documento/log bruto. Endpoint `GET /api/v1/ai/status` + componente reutilizável de aviso ("só dados sintéticos ou demonstrativos"; nunca expõe a chave). **Validação:** `dotnet build -c Release` **0/0**; backend **704/704** (SQLite, exceto a bateria PostgreSQL); `ng build` aprovado (warnings de budget CSS conhecidos); `git diff --check` limpo; sem segredo no diff; **SEM migration (28)**. **PRs #31 e #32 preservados como CONCLUÍDOS.** Chave Gemini e tenant Entra sandbox PENDENTES pós-merge. Próximo: deploy COMBINADO (PRs #31 + #32 + este) + criação/allowlisting do tenant Entra sandbox e smoke manual com Gemini real.
