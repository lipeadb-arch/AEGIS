...
**Data de atualização:** 2026-08-05<br>
**Horizonte desta revisão:** entrega funcional em 30 dias, até 2026-08-28<br>
**Branch de referência:** `main`<br>
**Commit de referência:** `24e8e61`<br>
**Última entrega concluída:** AEGIS KNIGHT — coletor REAL do Google Workspace (somente leitura) — PR #25, squash-merge `24e8e61` (Entra/multicoletor em `ea38bd6`/PR #24; fundação em `d53beb3`; MVP acelerado Entregas 0–5 já concluído)<br>
...
**Trabalho em curso (NÃO mergeado):** o primeiro pacote pós-MVP — **snapshots auditáveis de postura** (`AEGIS-AUD-035/036/037`) — está **EM IMPLEMENTAÇÃO** na branch `feat/aegis-auditable-history` (a partir de `24e8e61`): fotografia IMUTÁVEL COMPARTILHADA entre AEGIS Score/NIST e AEGIS KNIGHT (sem score combinado), publicação controlada por papel **Manager/TenantAdmin**, universo NIST COMPLETO com "não avaliado" ≠ 0 e ≠ NonCompliant, proveniência da evidência DECISIVA, hash determinístico INEQUÍVOCO, imutabilidade reforçada por **gatilho append-only no PostgreSQL**, comparação só entre compatíveis (senão estado explícito de incompatibilidade) e tela compartilhada `/history`. Migration ADITIVA `20260811143617_Auditable_Posture_Snapshots` (a **27ª**). Validação real: `dotnet build` **0/0**, backend **610/610**, DbMigrator em banco descartável (migrate+seed+verify+`--verify-only`; `aegis_dev` intocado), **teste PostgreSQL real do gatilho** aprovado, `ng build` aprovado, **smoke visual** do histórico. **Não concluir `AEGIS-AUD-035/036/037` antes do merge; PDF/CSV/exportação (`AEGIS-AUD-034`/restante de `AEGIS-AUD-040`) ficam para o pacote seguinte.**
