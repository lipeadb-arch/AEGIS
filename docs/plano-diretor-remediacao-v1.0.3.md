...
**Primeiro pacote pós-MVP CONCLUÍDO — snapshots auditáveis de postura (`AEGIS-AUD-035`, `AEGIS-AUD-036`, `AEGIS-AUD-037`): PR #26, squash-merge `dab5e6e`** (árvore idêntica ao head validado `47a5cd5`; branch `feat/aegis-auditable-history` removida local e remota). Fotografia IMUTÁVEL COMPARTILHADA entre AEGIS Score/NIST e AEGIS KNIGHT (sem score combinado), publicação controlada por papel **Manager/TenantAdmin**, universo NIST COMPLETO com "não avaliado" ≠ 0 e ≠ NonCompliant, proveniência da evidência DECISIVA, hash determinístico INEQUÍVOCO, imutabilidade reforçada por **gatilho append-only no PostgreSQL**, comparação só entre compatíveis (senão estado explícito de incompatibilidade) e tela compartilhada `/history` (histórico, detalhe e comparação). Migration ADITIVA `20260811143617_Auditable_Posture_Snapshots` (a **27ª**). O mesmo PR corrige a **corrida concorrente no upsert do ledger** (`ControlStateWriter`): recuperação pontual da inserção perdida, reaplicando o veredito como UPDATE sem duplicar (sem migration). Validação real: `dotnet build` **0/0**, backend **611/611**, **bateria PostgreSQL 20/20** (gatilho + corrida do ledger + `Concurrent_MesmoEvento` aprovado 3×), DbMigrator em banco descartável (`aegis_dev` intocado), `ng build` e **smoke visual** do histórico aprovados. **Exportação executiva PDF/CSV (`AEGIS-AUD-034`) EM IMPLEMENTAÇÃO** — branch `feat/aud-034-executive-export` (PR aberto, NÃO mergeado): endpoint `GET /api/v1/posture/snapshots/{id}/export?format=pdf|csv` com PDF executivo pt-BR (PDFsharp-MigraDoc 6.2.0, edição CORE/MIT, sem GDI/Office) e CSV (UTF-8+BOM, `;`, protegido contra CSV/Formula Injection), ambos derivados EXCLUSIVAMENTE da `PostureSnapshot` imutável (tenant implícito, hash reverificado → 404/400/409); botões Baixar PDF/CSV no detalhe de `/history`; **sem migration**, sem mudança de fórmula/catálogo/ledger/Dashboard. Validação: `dotnet build` 0/0, **backend 622/622** (+11 focados, incl. equivalência detalhe×PDF×CSV), `ng build` aprovado, `has-pending-model-changes` limpo (27 migrations), smoke visual dos PDFs/CSVs. **`AEGIS-AUD-040` PARCIAL**: relatório da MESMA projeção publicada atendido; Dashboard ao vivo ligado a snapshot fica como resíduo (fora do escopo). **Próximo pacote técnico após o merge: resíduo do `AEGIS-AUD-040`, se autorizado.**
...
`AEGIS-AUD-039`, `AEGIS-AUD-040`. (`AEGIS-AUD-035/036/037` CONCLUÍDOS no PR #26 — snapshots auditáveis; `AEGIS-AUD-034` **EM IMPLEMENTAÇÃO** — exportação executiva PDF/CSV, branch `feat/aud-034-executive-export`, PR aberto; `AEGIS-AUD-040` **PARCIAL** — relatório derivado da mesma projeção publicada atendido, Dashboard ao vivo ligado a snapshot pendente.)
