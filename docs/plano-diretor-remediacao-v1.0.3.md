...
**Pacote técnico pré-tenant — evidência documental rastreável, retração/reconciliação, status ao vivo e terminologia — PR #31 ABERTO (2026-08-16), NÃO mergeado** (branch `fix/document-evidence-lifecycle-live-ux`, base `origin/main == 7a1d7af`). Corrige os defeitos vistos na homologação antes de conectar um tenant real: um documento sem prova literal deixava o **Govern em 40%** (o Stub fabricava GV.PO-01 "aprovada pela direção"; o worker projetava a triagem crua; o DELETE deixava cobertura/ledger órfãos; não havia status ao vivo). Agora **(A)** exige-se `evidenceQuote` LITERAL validado em código e projeta-se só resultados refinados+validados (refinamento indisponível → retry durável, nunca triagem-como-prova; documento sintético → zero claims; teto documental de 50%); **(B)** uma rotina ÚNICA de reconciliação (exclusão/reanálise) retrai/recalcula preservando telemetria e entrevista, com a migration `20260816192614_DocumentEvidenceLifecycle` (a **28ª**) + REPARO idempotente pelo `DbMigrator` que zera o Govern órfão no próximo deploy e re-enfileira documentos existentes; **(C)** o Document Hub faz polling (~2s, `timer`+`exhaustMap`, para no estado terminal) e atualiza os agregados na conclusão e na exclusão sem refresh; **(D)** terminologia visível padronizada (nomes de produto/NIST/códigos preservados). **Validação:** `dotnet build -c Release` **0/0**, `has-pending-model-changes` limpo (**28 migrations**), backend focado **SQLite 80/80**, teste PostgreSQL do reparo gated por `AEGIS_TEST_PG`, `ng build` aprovado, `git diff --check` limpo, sem segredo. **Neon/Render intocados; sem chave de IA; sem merge.** ⚠️ `DbMigrator` contra PostgreSQL descartável (incl. `--verify-only`) NÃO executado nesta sessão (credencial local inacessível ao agente) — coberto por `has-pending-model-changes`, teste PG gated e `container-smoke`. Próximo: redeploy de homologação → confirmar **Govern = Não avaliado** → prosseguir para o tenant real. PR: https://github.com/lipeadb-arch/AEGIS/pull/31.
