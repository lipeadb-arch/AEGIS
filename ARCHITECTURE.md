...
S.T.A.R.S é um **motor de IA para diagnóstico contínuo de maturidade cibernética** em ambientes corporativos multi-cliente. Ele substitui o processo manual de auditoria (planilhas + questionários de autodeclaração + entrevistas) por um ciclo automatizado que:
...
A frase-guia da tese GRC resume o propósito do produto: o risco cibernético só vira decisão quando *vulnerabilidade, ativo, processo, controle, compliance e auditoria falam a mesma língua*. S.T.A.R.S é a camada de tradução que produz essa língua comum.

---

## 2. Problema atual → o que S.T.A.R.S muda

| Hoje (modelo manual) | Com S.T.A.R.S |
|---|---|
...
S.T.A.R.S não substitui o julgamento do analista nem mistura as áreas: é um **validador** entre a diretriz teórica e a prática operacional, **preservando a independência das linhas de defesa** (ver §3).
...
A **convergência** entre autodeclaração, documento e fato de API é o que diferencia S.T.A.R.S de um questionário comum: a IA aponta divergências (ex.: BU declara "MFA habilitado" mas Secure Score Identity = 31%) e ajusta a confiança/recomendação.
...
Para **não depender de agentes proprietários** (EDR específico) nem reimplementar coletores frágeis, os adapters reaproveitam ferramentas open-source consolidadas como *motor de coleta*; o S.T.A.R.S apenas **normaliza** a saída para `EvidenceSignal`:
...
Stars.sln
└─ src/
   ├─ Stars.Domain            // entidades, enums, regras puras (sem dependências)
   ├─ Stars.Application        // serviços de aplicação, interfaces (IAiAssessmentService,
   │                           //   IEvidenceConnector, IConnectorRegistry), scoring
   ├─ Stars.Infrastructure     // EF Core (StarsDbContext), seeder, AI (Claude), cripto, tenant ctx
   ├─ Stars.Connectors.Microsoft  // plugin de conectores Microsoft (Secure Score, Defender...)
   └─ Stars.Api                // ASP.NET Core: Program.cs, controllers, DTOs, auth, DI
data/
...
- **LGPD**: minimização (coletamos *postura*, não dados pessoais do cliente final), retenção configurável, e o próprio S.T.A.R.S monitora GV.OC-03 (obrigações legais).
...
S.T.A.R.S é um módulo do portal Synapse OS:
...
- **Backend .NET**: `Stars.Domain` (modelo completo), `Stars.Application` (interfaces + serviços de pontuação Maturidade/Risco/ICR), `Stars.Infrastructure` (`StarsDbContext`, seeder, serviço de IA Claude), `Stars.Connectors.Microsoft` (conector Secure Score de exemplo), `Stars.Api` (Program + controllers principais + DTOs).
