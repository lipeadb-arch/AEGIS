...
> **Propósito deste arquivo:** reinjeção de contexto exato em futuras sessões de IA, sem reler o código-fonte. Não é documentação comercial. Última atualização: **2026-08-01**.
>
...
>
> **Estado da Entrega 4 — Workspace NIST, Dashboard e Document Hub (EM IMPLEMENTAÇÃO, 2026-08-01): `AEGIS-AUD-021`, `AEGIS-AUD-027`, `AEGIS-AUD-032` — NÃO mergeada.** Branch `feat/mvp-nist-workspace` (PR aberto contra `main`, **não mergeado** — não marcar os AUDs antes do merge). Nova **projeção ÚNICA** de leitura `GET /api/v1/scoring/workspace` (`IWorkspacePostureQuery`/`WorkspacePostureQuery`): postura GERAL e por Função NIST (score anulável + cobertura pela fórmula `aegis-score-v1`, restrita ao framework ATIVO) + saúde/recência dos conectores — a MESMA autoridade consumida pelo Dashboard e pelas seis Funções, sem recomputo concorrente. Componente compartilhado `PostureSummaryComponent` (contrato único) no Dashboard executivo (banda AEGIS Score, distinta da maturidade CMMI/ICR) e nas seis Funções (PR/DE/RS/RC via `PillarDashboard`, GV no Document Hub, ID no Inventário de Ativos); `NotEvaluated` NUNCA exibido como 0%; conector nunca sincronizado ≠ saudável. Document Hub e isolamento no switch (cancelar + reload do `TenantContextService`) já estavam completos — reaproveitados. **Sem migration** (leitura pura sobre o modelo atual; `has-pending-model-changes` limpo, 24 migrations). Backend **536/536** (+8 testes focados da projeção: isolamento por tenant, só framework ativo, consistência score/cobertura/contagens, NotEvaluated ≠ 0%, paridade Função↔total, conector nunca sincronizado); `dotnet build` 0 erros/0 warnings; `ng build` aprovado (4 warnings de budget CSS conhecidos). Próximo passo: revisão e merge do PR; só então marcar os AUDs e a Entrega 4 como CONCLUÍDA.
>
