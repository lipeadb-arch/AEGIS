...
> **Propósito deste arquivo:** reinjeção de contexto exato em futuras sessões de IA, sem reler o código-fonte. Não é documentação comercial. Última atualização: **2026-07-27**.
>
...
>
> **Em implementação (2026-07-27, PRÉ-MERGE — NÃO concluído): `AEGIS-AUD-009`** — armazenar somente hash de refresh tokens (EP-01), na branch `fix/aud-009-hash-refresh-tokens`. O `UserRefreshToken` deixa de persistir o token bruto: `Token`→**`TokenHash`** e `ReplacedByToken`→**`ReplacedByTokenHash`**, ambos **SHA-256 hex de 64 chars** calculado por um único componente **`Sha256RefreshTokenHasher`** (`IRefreshTokenHasher`, SHA-256 puro — sem pepper/HMAC, que seriam escopo do AUD-060). O bruto existe só na geração, no `TokenPair` interno e no cookie HttpOnly. Emissão/refresh/rotação/logout/troca de tenant/revogação de cadeia operam **exclusivamente por hash** (índice único tenant-leading `(TenantId, TokenHash)`). A janela de idempotência deixou de reentregar o sucessor bruto (impossível a partir do hash): o refresh agora tem **desfecho explícito** (`RefreshResult`/`RefreshOutcome` = Success · InvalidOrBreach · RotationConflict) e o conflito benigno vira **HTTP 409 + Retry-After** (o front tenta **uma** vez com o cookie sucessor já atualizado pelo vencedor), **sem revogar a cadeia nem limpar o cookie**; breach/expirado/ inválido seguem 401 + limpeza. Rotação atômica (`UPDATE … WHERE RevokedAt IS NULL`) intacta — um só vencedor, sem fork. Migration **`20260727204000_Aud009HashRefreshTokens`** faz **backfill SHA-256 no PostgreSQL** (`encode(sha256(convert_to(v,'UTF8')),'hex')`, nativo desde o PG 11, sem pgcrypto — preserva sessões existentes); `Down` invalida sessões (apaga só as linhas de refresh) sem reintroduzir plaintext. `aegis_dev` migrado **19→20** (152 linhas legadas hasheadas, zero plaintext); `has-pending-model-changes` limpo; DbMigrator normal e `--verify-only` aprovados. Baseline **337 → 352, 352/352** (inclui concorrência/rotação e migration sobre schema anterior em **PostgreSQL descartável real**, gate `AEGIS_TEST_PG`); frontend `ng build` exit 0 (mesmos 4 warnings de budget). **Não mergeado — não concluído.**
>
