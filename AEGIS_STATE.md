...
>
> **Em implementação (2026-07-28, PRÉ-MERGE — NÃO concluído): `AEGIS-AUD-007`** — autenticação corporativa federada (Microsoft Entra ID), EP-01, na branch `fix/aud-007-federated-auth`. O Entra apenas AUTENTICA a identidade; após validar o token externo, o AEGIS emite o par LOCAL já existente (JWT + refresh HttpOnly por hash) usando o membership interno — **nada substitui o JWT local, o isolamento por tenant nem o login local**. Fluxo: SPA obtém o token do Entra via **MSAL (`@azure/msal-browser`, Auth Code + PKCE, cache só em memória)** e o envia a `POST /auth/federation/exchange`, protegido por um **esquema JWT Bearer SEPARADO** que valida assinatura/issuer/audience/lifetime via JWKS do tenant; o serviço nunca confia em identidade vinda do corpo. **Modos `Auth:Federation:Mode` = Local (padrão, dev/demonstração) · Federated (só corporativo; senha desabilitada) · Hybrid (ambos)**, com fail-fast de config incompleta no startup; config pública sanitizada em `GET /auth/federation/config` (nunca segredo). **Vínculo por `tid+oid` imutáveis** (campos nullable em `IdentityAccount` + índice único parcial): primeiro login só vincula conta preexistente com e-mail correspondente e ainda não vinculada; **sem auto-provisionamento** de conta/membership/tenant/papel (é o AUD-010); troca de e-mail no Entra não quebra o login; e-mail não captura conta já vinculada a outro oid. Migration aditiva **`20260728212552_Aud007FederatedIdentityBinding`** (2 colunas nullable + índice único parcial; `Down` só remove o vínculo, preserva contas/memberships/sessões); `aegis_dev` **20→21 migrations**; `has-pending-model-changes` limpo; DbMigrator normal e `--verify-only` aprovados. Backend **352→381, 381/381** (inclui unicidade do vínculo e migration/rollback em **PostgreSQL descartável real**); frontend `ng build` exit 0 (mesmos 4 warnings); **sem tocar Sentinel/telemetria/Secure Score**. **Não mergeado — não concluído.**
>
