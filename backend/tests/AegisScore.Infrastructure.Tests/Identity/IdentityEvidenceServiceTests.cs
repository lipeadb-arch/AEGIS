...
public sealed class IdentityEvidenceServiceTests : IDisposable
{
    ...
    private IIdentityEvidenceService ServiceFor(AegisScoreDbContext db, Guid tenantId, IKnightCollector collector)
    {
        ...
        return new IdentityEvidenceService(db, registry, config, new AegisScore.Infrastructure.Identity.IdentityAcquisitionStore(db, new SystemTenantContext(tenantId), TimeProvider.System), new SystemTenantContext(tenantId));
    }
