...
/// <summary>
/// Roteadores tenant-scoped (a fronteira de dados na prática): provam a SELEÇÃO Gemini × Stub por tenant e a
/// invariante crítica de segurança — um tenant fora da allowlist NUNCA chega a fazer uma chamada HTTP externa.
/// Sem rede real: o motor externo é um <see cref="GeminiLlmClient"/> sobre um handler-espião.
/// </summary>
public sealed class TenantScopedAiRouterTests
{
    ...
    [Fact]
    public async Task LlmRouter_TenantAllowlisted_UsaGemini()
    {
        var handler = new SpyHandler(HttpStatusCode.OK, GeminiResponse("resposta-real"));
    ...
    {
        var handler = new SpyHandler(HttpStatusCode.OK, GeminiResponse("nao-deveria-vir"));
    ...
    {
        var handler = new SpyHandler(HttpStatusCode.OK, GeminiResponse("x"));
    ...
    {
        var handler = new SpyHandler(HttpStatusCode.OK, GeminiResponse("{}"));
    ...
    [Fact]
    public async Task AssessmentRouter_TenantAllowlisted_UsaGemini()
    {
        var handler = new SpyHandler(HttpStatusCode.OK, GeminiResponse("""{"summary":"real","claims":[]}"""));
    ...
    private static TenantScopedLlmRouter LlmRouter(string slug, SpyHandler handler, string allow, string apiKey = "chave") =>
        new(Gemini(handler, apiKey), new StubLlmClient(), GateFor(apiKey, allow), new FakeResolver(slug));
    ...
    private static TenantScopedAssessmentRouter AssessmentRouter(string slug, SpyHandler handler, string allow, string apiKey = "chave") =>
        new(
            new AegisAssessmentService(Gemini(handler, apiKey), StaticAuditorPersonaProvider.Neutral),
    ...
    private static AiFreeTierGate GateFor(string apiKey, string allow) =>
        ...
        {
            Mode = AiMode.GeminiFreeDemo,
            ApiKey = apiKey,
            FreeTier = new AiFreeTierOptions { AllowedTenantSlugs = { allow } },
        }));

    private static GeminiLlmClient Gemini(SpyHandler handler, string apiKey) =>
