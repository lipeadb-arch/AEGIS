...
public sealed class CrossSourceCorrelationQuery : ICrossSourceCorrelationQuery
{
    ...
    private const string VulnerabilitiesLabel = "Microsoft Defender Vulnerability Management";
    private const string DeviceManagementLabel = "Microsoft Intune";
    ...
    private sealed record AssetRow(Guid Id, string Name, AssetNameOrigin NameOrigin);
    ...
    private async Task<Dictionary<Guid, CrossSourceConnectorFacts>> LoadConnectorsAsync(CancellationToken ct)
    {
        ...
        var postures = ids.Count == 0
            ...
            : (await _db.DevicePostureSnapshots.AsNoTracking()
    ...
    /// </summary>
    private async Task<Dictionary<Guid, CrossSourceAssetFacts>> LoadFactsAsync(
        IReadOnlyList<AssetRow> assets, IReadOnlyDictionary<Guid, CrossSourceConnectorFacts> connectors, CancellationToken ct)
    {
        ...
        foreach (var chunk in assets.Chunk(Chunk))
        {
            ...
            var bindings = await _db.AssetSourceBindings.AsNoTracking()
                ...
                .ToListAsync(ct);
            var keys = await _db.AssetStrongIdentifiers.AsNoTracking()
            ...
            var groups = await (
                    ...
                    join e in _db.AssetThreatExposures.AsNoTracking() on o.AssetThreatExposureId equals e.Id
    ...
    /// </summary>
    private async Task<T> InReadSnapshotAsync<T>(Func<Task<T>> read, CancellationToken ct)
    {
        if (_db.Database.CurrentTransaction is not null) return await read();
        var npgsql = _db.Database.IsNpgsql();
        await using var tx = npgsql
            ? await _db.Database.BeginTransactionAsync(IsolationLevel.RepeatableRead, ct)
            : await _db.Database.BeginTransactionAsync(ct);
        if (npgsql) await _db.Database.ExecuteSqlRawAsync("SET TRANSACTION READ ONLY", ct);
