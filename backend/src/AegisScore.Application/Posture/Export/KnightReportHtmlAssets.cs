...
.filters{display:grid;grid-template-columns:2fr repeat(5,minmax(0,1fr)) auto;gap:8px;align-items:end}
...
var ST={tab:'overview',f:{q:'',status:'',severity:'',service:'',domain:'',framework:''}};
...
var pr=el('div',{cls:'panel'},[el('h2',{text:'Riscos prioritários'}),el('p',{cls:'sub',text:'Até cinco controles reprovados, ordenados por severidade e, depois, por objetos afetados. Nenhum critério além do que a avaliação comprovou.'})]);
...
box.appendChild(pr);
var to=el('div',{cls:'panel'},[el('h2',{text:'Principais objetos afetados'}),el('p',{cls:'sub',text:'Objetos que aparecem em mais controles reprovados ou mitigados.'})]);
if(D.topObjects.length===0)to.appendChild(el('p',{cls:'empty',text:D.kpis.uniqueAffected===null?'Esta fotografia não congelou os objetos afetados.':'Nenhum objeto afetado preservado.'}));
...
to.appendChild(el('div',{cls:'tw'},[el('table',null,[el('thead',null,[el('tr',null,[el('th',{text:'Objeto'}),el('th',{text:'Tipo'}),el('th',{text:'Controles'}),el('th',{text:'Quais'})])]),tb])]));}
box.appendChild(to);
...
el('span',{cls:'k',text:'Escopo desta entrega'}),el('span',{text:'Identidade ('+h.sourceLabel+'). Microsoft 365/Teams e Azure ainda não fazem parte desta avaliação e não estão declarados como cobertos.'})]));
...
p.appendChild(el('p',{cls:'sub',text:'Este arquivo contém a avaliação completa (todos os controles e objetos congelados), não um recorte. O CSV da mesma fotografia tem uma linha por objeto de cada controle (ou uma linha para o controle sem objetos): controles = valores distintos de IndicatorId; ocorrências = linhas com ObjectRelation "Afetado" em controles reprovados ou mitigados; objetos únicos = pares distintos (ObjectType, ObjectExternalId) dessas linhas.'}));
...
var q=el('input',{id:'f-q',type:'search',placeholder:'Controle, código, objeto…',oninput:function(){ST.f.q=q.value;renderList();}});inputs.q=q;
...
if(f.severity&&c.severity!==f.severity)return false;if(f.service&&c.service!==f.service)return false;if(f.domain&&c.domain!==f.domain)return false;
...
if(f.service)p.push('serviço: '+f.service);if(f.domain){var d=D.controls.filter(function(c){return c.domain===f.domain;})[0];p.push('domínio: '+(d?d.domainLabel:f.domain));}
...
recorte.textContent='Mostrando '+list.length+' de '+D.controls.length+' controle(s) · Recorte: '+describeFilter();
...
[el('span',null,[el('div',{cls:'t',text:c.title}),el('div',{cls:'s',text:c.id+' · '+c.service+' · '+c.domainLabel+(c.affectedCount>0?' · '+c.affectedCount+' afetado(s)':'')})]),
...
b.appendChild(sec('O problema',[el('p',{text:c.description||'Descrição não congelada nesta fotografia.'}),el('div',{cls:'kv'},[el('span',{cls:'k',text:'Constatação desta avaliação'}),el('span',{text:c.evidence}),
...
b.appendChild(sec('Por que importa',[el('p',{text:c.rationale||'Impacto não congelado nesta fotografia.'}),c.doesNotProve?el('p',{cls:'note',text:'O que este resultado não comprova: '+c.doesNotProve}):null]));
b.appendChild(sec('Onde foi encontrado',[objects(c)]));
...
b.appendChild(sec('Evidência técnica e proveniência',[el('div',{cls:'kv'},[el('span',{cls:'k',text:'Provedor · serviço · domínio'}),el('span',{text:c.provider+' · '+c.service+' · '+c.domainLabel}),
...
el('span',{cls:'k',text:'Frameworks'}),el('span',{text:fw.length?fw.join(' · '):'—'}),el('span',{cls:'k',text:'Capacidades de coleta usadas'}),el('span',{cls:'mono',text:c.requiredCapabilities.join(', ')||'—'}),
...
var info=[];if(c.affectedCount>0)info.push(c.affectedCount+' objeto(s) afetado(s) segundo a regra');if(c.evidenceCount>0)info.push(c.evidenceCount+' evidência(s) de configuração');
if(c.detailPreserved===null)wrap.appendChild(el('p',{cls:'note',text:'Esta fotografia (v1) não congelou objetos. A lista de hoje não serve de prova para um resultado anterior.'}));
else if(!c.objects.length)wrap.appendChild(el('p',{cls:'empty',text:c.affectedCount>0?'Objetos não preservados para este controle.':'Nenhum objeto associado a este resultado.'}));
...
var q=all.length>10?el('input',{type:'search',placeholder:'Filtrar objetos deste controle…','aria-label':'Filtrar objetos de '+c.id,oninput:function(){filter=q.value.toLowerCase();draw();}}):null;
...
if(q)wrap.appendChild(q);wrap.appendChild(el('div',{cls:'tw'},[el('table',null,[el('thead',null,[el('tr',null,['Relação','Tipo','Objeto','Por que está aqui','Configuração observada'].map(function(x){return el('th',{text:x});}))]),tb])]));wrap.appendChild(more);draw();return wrap;}
// ---------- Montagem ----------
